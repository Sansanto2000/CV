# CV

En la carpeta `Congresos-y-Eventos` se guardan los certificados de Congresos/Eventos en los que se participó de alguna forma. A continuación se resume la información disponible.

|Congreso/Evento    |Año    |Asistente  |Autor  |Conferencista  |
|:------------------|:-----:|:---------:|:-----:|:-------------:|
|ExperienciasUNLP   |2026   |🟢         |       |🟢             |
|JCC                |2026   |🟢         |🟢      |🟢             |
|WICC               |2026   |           |🟢2️⃣    |               |
|CLEI               |2025   |🟢         |🟢     |🟢             |
|ExperienciasUNLP   |2025   |🟢         |       |🟢             |
|WICC               |2025   |           |🟢     |               |
|KHIPU              |2025   |🟢         |       |               |
|WICC               |2024   |           |🟢     |               |
|III_CIMA           |2024   |🟢         |       |🟢             |
|Enigma             |2024   |🟢         |       |🟢             |
|AUGM_JJI           |2024   |🟢         |🟢     |🟢             |
|CACIC              |2024   |🟢         |🟢     |               |

## Cursos

En la carpeta `Cursos` se guarda información de cursos realizados ya sea en contexto de un congreso o realizado por la facultad.

|Institución/Contexto   |Año    |Curso              |Aprobado   |
|:----------------------|:-----:|:-----------------:|:---------:|
|CLEI                   |2025   |*Redes Neuronales* |🟢         |
|CACIC                  |2025   |*Sistemas de Ciberdefensa con IA*|🟢        |
|CACIC                  |2024   |*Gemelos digitales y su aplicación en la industria y en la gestión sanitaria*|🟢        |
|UNLP_FI                |2024   |*ANÁLISIS INTELIGENTE DE DATOS EN ENTORNOS BIG DATA*|🟢        |
|CACIC                  |2023   |*Redes Neuronales con Python*|🟢        |

## Investigador

La carpeta `Investigador` persiste constancias relacionadas a hitos académicos. Resumidos en la siguiente tabla.

|Hito                       |Año    |Descripción    |
|:--------------------------|:-----:|:--------------|
|*Egreso de Maestría en Inteligencia de Datos Orientada en Big Data*    |2026   |Inicio en 2024. En documentos se puede encontrar el certificado analitico.  |
|*Beca Doctoral UNLP*       |2023   |Otorgamiento de la Beca Doctoral UNLP solicitada en 2022.  |
|*Beca EVC*                 |2022   |Otorgamiento de la Beca *Estímulo a las Vocaciones Científicas* solicitada en 2021, resuelta simultáneamente a la tesis de grado.  |

___

**IMPORTANTE**: No está incluida la información de todos los Congresos\Eventos\Cursos en los que se participó. Algunos se perdieron o no recibieron sus respectivos certificados por lo que nunca se agregaron sus entradas en el repositorio. 
